...
test('update-nix-release writes version and AppImage hashes', () => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'magiesTerminal-nix-release-'));
  ...
  fs.writeFileSync(path.join(artifacts, 'MagiesTerminal-1.2.3-linux-x86_64.AppImage'), x64);
  fs.writeFileSync(path.join(artifacts, 'MagiesTerminal-1.2.3-linux-arm64.AppImage'), arm64);
