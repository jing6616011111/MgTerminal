...
脚本会自动识别当前用户：`root` 直接安装，普通用户则在需要时调用 `sudo`。Web 版包含管理员登录、加密凭据存储、SSH 主机指纹确认和实时 WebSocket 终端。公网使用前必须配置 HTTPS；完整说明见 [`docs/WEB_DEPLOYMENT.zh-CN.md`](docs/WEB_DEPLOYMENT.zh-CN.md)。
