...
<p align="center">
  <a href="https://github.com/JasonZhangDad/MgTerminal/releases/latest">
    <img src="https://img.shields.io/github/v/release/JasonZhangDad/MgTerminal?style=for-the-badge&logo=github&label=下载最新版&color=success" alt="下载最新版">
  </a>
</p>
...
<a name="magiesTerminal-是什么"></a>
# MagiesTerminal 是什么

**MagiesTerminal** 是一款现代化的跨平台 SSH 客户端和终端管理器，专为需要高效管理多台远程服务器的开发者、系统管理员和 DevOps 工程师设计。

- **MagiesTerminal 是** PuTTY、Termius、SecureCRT 和 macOS Terminal.app 的现代替代品
- **MagiesTerminal 是** 一个强大的 SFTP 客户端，支持双窗格文件浏览
- **MagiesTerminal 是** 一个终端工作空间，支持分屏、标签页和会话管理
- **MagiesTerminal 支持** SSH、本地终端、Telnet、Mosh、串口（Serial）等连接方式（视环境而定）
- **MagiesTerminal 不是** Shell 替代品 —— 它通过 SSH/Telnet/Mosh 或本地/串口会话连接到 Shell

---

<a name="为什么是-magiesTerminal"></a>
# 为什么是 MagiesTerminal

如果你需要同时维护多台服务器，MagiesTerminal 更像是“工作台”而不是单一终端：
...
MagiesTerminal 会自动识别并在主机列表中展示对应的系统图标：
...
从 [GitHub Releases](https://github.com/JasonZhangDad/MgTerminal/releases/latest) 下载适合您平台的最新版本。
...
或在 [GitHub Releases](https://github.com/JasonZhangDad/MgTerminal/releases) 浏览所有版本。

### Web 服务器版（本 Fork 新增）

在 Linux 服务器一键部署，通过浏览器登录并管理 SSH 主机：

```bash
curl -fsSL https://raw.githubusercontent.com/jing6616011111/MgTerminal/main/scripts/install-web.sh | bash
```

脚本会自动识别当前用户：`root` 直接安装，普通用户则在需要时调用 `sudo`；安装完成后同时显示内网地址和公网候选地址。Web 版包含管理员登录、加密凭据存储、SSH 主机指纹确认和实时 WebSocket 终端。公网使用前必须配置 HTTPS；完整说明见 [`docs/WEB_DEPLOYMENT.zh-CN.md`](docs/WEB_DEPLOYMENT.zh-CN.md)。

> Web 版当前聚焦主机管理与 SSH 终端，Electron 客户端中的 SFTP、端口转发、AI Agent 等高级能力将分阶段迁移。

> **macOS 用户注意：** 当前发布版本应已完成代码签名和公证。如果 Gatekeeper 仍然提示风险，请确认您下载的是 GitHub Releases 中的最新官方构建。

### Nix / NixOS

MagiesTerminal 提供了一个 flake，为 Nix 和 NixOS 用户封装了官方 Linux AppImage 发行版：

```bash
nix run github:JasonZhangDad/MgTerminal
```

声明式安装时，将 MagiesTerminal flake 添加为输入，并在 NixOS 或 Home Manager 的软件包列表中使用 `inputs.magiesTerminal.packages.${pkgs.system}.default`。
...
<p align="center">
  用 ❤️ 制作，作者 <a href="https://github.com/JasonZhangDad">JasonZhangDad</a>
</p>
