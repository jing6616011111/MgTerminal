#!/usr/bin/env bash
#
# bump-homebrew-cask.sh — push a new version of the MagiesTerminal cask to the
# JasonZhangDad/homebrew-magies-terminal tap.
#
...
#
# Optional env vars:
#   TAP_REPO             — default: JasonZhangDad/homebrew-magies-terminal
#   ARTIFACTS_DIR        — default: artifacts
#   CASK_PATH            — default: Casks/magiesTerminal.rb
set -euo pipefail

: "${VERSION:?VERSION env var required (no leading v)}"
: "${HOMEBREW_TAP_TOKEN:?HOMEBREW_TAP_TOKEN env var required}"

TAP_REPO="${TAP_REPO:-JasonZhangDad/homebrew-magies-terminal}"
ARTIFACTS_DIR="${ARTIFACTS_DIR:-artifacts}"
CASK_PATH="${CASK_PATH:-Casks/magiesTerminal.rb}"

ARM_DMG="${ARTIFACTS_DIR}/MagiesTerminal-${VERSION}-mac-arm64.dmg"
X64_DMG="${ARTIFACTS_DIR}/MagiesTerminal-${VERSION}-mac-x64.dmg"
...
git add "$CASK_PATH"
git commit -m "Bump magiesTerminal to ${VERSION}"
