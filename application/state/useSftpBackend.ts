...
export const useSftpBackend = () => {
  const openSftp = useCallback(async (options: MagiesTerminalSSHOptions) => {
  ...
  const listDrives = useCallback(async () => {
    return await magiesTerminalBridge.get()?.listDrives?.() ?? [];
  }, []);
  ...
  const startStreamTransfer = useCallback(
    async (
      options: Parameters<NonNullable<MagiesTerminalBridge["startStreamTransfer"]>>[0],
      onProgress?: (transferred: number, total: number, speed: number) => void,
      onComplete?: () => void,
      onError?: (error: string) => void,
    ) => {
  ...
  const onTransferProgress = useCallback((transferId: string, cb: Parameters<NonNullable<MagiesTerminalBridge["onTransferProgress"]>>[1]) => {
    const bridge = magiesTerminalBridge.get();
