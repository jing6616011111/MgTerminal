...
export function useSftpTransferConflictOps() {
  ...
  const statTargetPath = useCallback(
    async (
      ...
      if (targetPane.connection.isLocal) {
        const stat = await magiesTerminalBridge.get()?.statLocal?.(targetPath);
        if (!stat) return null;
        return {
  ...
  const deleteTargetPath = useCallback(
    async (
      ...
      if (targetPane.connection.isLocal) {
        const deleteLocalFile = magiesTerminalBridge.get()?.deleteLocalFile;
      ...
      const deleteSftp = magiesTerminalBridge.get()?.deleteSftp;
