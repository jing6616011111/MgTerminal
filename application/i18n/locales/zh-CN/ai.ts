...
export const zhCNAiMessages: Messages = {
  ...
  'ai.providers.contextWindow.help': '留空时优先使用模型列表返回的值；如果没有，MagiesTerminal 会使用安全默认值。',
  ...
  'ai.codex.customConfigMissingEnvKey': '警告：环境变量 {envKey} 未在当前 shell 中设置。请 export 它（或从包含该变量的 shell 启动 magiesTerminal），否则 Codex 无法鉴权。',
  ...
  'ai.externalMcp.description': '把 MagiesTerminal 作为 MCP 服务器暴露给 Codex、Claude Code、Cursor、Grok 等外部客户端。工具面与应用内 Agent 相同（终端、SFTP、Vault、端口转发）。客户端连接期间请保持 MagiesTerminal 运行。',
  ...
  'ai.externalMcp.mode.description': '临时模式会在空闲超时后自动关闭；常开模式会在 MagiesTerminal 启动时恢复对外 MCP。',
  ...
  'ai.externalMcp.usage.keepRunning': '1. 打开对外 MCP 开关，并保持 MagiesTerminal 运行。',
  ...
  'ai.externalMcp.permissionMode.hint': '与「设置 → AI → 安全」为同一项。自动：MagiesTerminal 写操作不再弹本应用审批；确认：每次询问。外部客户端（Codex / Claude / Grok）仍可能有自己的工具审批界面。',
  ...
  'ai.externalMcp.conflict.description': '已存在指向其他位置的 magiesTerminal-external 条目，请手动删除或修改。',
  ...
  'ai.userSkills.description': '打开 MagiesTerminal 的 Skills 文件夹以添加你自己的技能目录。MagiesTerminal 会自动扫描这些 skills，默认只注入轻量索引，只有在请求明显命中某个 skill 时才展开正文。',
  ...
  // AI Error
  'ai.codex.bridgeError': 'Codex 主进程处理器尚未加载。请完全重启 MagiesTerminal 或重启 Electron 开发进程，然后重试。',
  ...
  'ai.safety.permissionMode.description': '控制 AI 通过 MagiesTerminal 访问终端会话的方式。观察者模式会阻止经由 MagiesTerminal 的写操作；外部 Agent CLI 可能仍有自己的本机工具和审批流程。',
  ...
  'ai.safety.commandTimeout.description': '通过 MagiesTerminal 执行命令时允许运行的最长秒数，超时将被终止。',
  ...
  'ai.safety.blocklist.description': '用于拦截通过 MagiesTerminal 执行的危险命令的正则表达式。',
  ...
  'ai.safety.note': '这些安全设置会约束经由 MagiesTerminal 执行的操作。外部 Agent CLI 也可能提供本机工具，那部分由 Agent 自己的控制规则约束。',
