...
export const zhCNTerminalMessages: Messages = {
  ...
  'terminal.osc7Setup.desc': 'MagiesTerminal 会为当前远端用户添加 OSC 7 提示符配置。这样在 sudo 或 su 之后，SFTP 也能继续跟随终端目录。',
  ...
  'terminal.et.proxyUnsupported': 'EternalTerminal 目前不支持 MagiesTerminal 的代理设置。请改用 SSH，或移除该主机的代理。',
  'terminal.et.multiJumpUnsupported': 'EternalTerminal 目前在 MagiesTerminal 中最多支持一个跳板机。',
  ...
  'settings.sshDeepLink.enable': '用 MagiesTerminal 打开 ssh:// 和 telnet:// 链接',
  'settings.sshDeepLink.enableDesc': '允许 MagiesTerminal 接管来自浏览器和其他应用的 ssh://、telnet:// 链接。',
  'settings.jmsDeepLink.title': 'JumpServer 链接',
  'settings.jmsDeepLink.enable': '处理 jms:// 链接',
  'settings.jmsDeepLink.enableDesc': '将 MagiesTerminal 注册为 JumpServer jms:// 客户端链接的处理程序，可能与官方 JumpServer 客户端冲突。',
  ...
  'snippets.import.toast.invalidDesc': '这不是有效的 MagiesTerminal 代码片段文件。',
  ...
  'snippets.import.modal.desc': '选择一个或多个 MagiesTerminal 代码片段 JSON 文件。导入前会先解析并展示结果，不会直接写入。',
