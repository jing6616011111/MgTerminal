...
export const enAiMessages: Messages = {
  ...
  'ai.providers.contextWindow.help': 'Leave blank to use the model list value when available, otherwise MagiesTerminal uses a safe default.',
  ...
  'ai.codex.customConfigMissingEnvKey': 'Warning: {envKey} is not set in your shell environment. Export it (or launch magiesTerminal from a shell that has it) so Codex can authenticate.',
  ...
  'ai.cursor.apiKeyEnvHint': 'Cursor can use CURSOR_API_KEY from your shell. Save a key here only if you want MagiesTerminal to override it.',
  'ai.cursor.apiKeyOverrideHint': 'MagiesTerminal will use the saved key here before CURSOR_API_KEY.',
  ...
  'ai.externalMcp.description': 'Expose MagiesTerminal as an MCP server for external clients such as Codex, Claude Code, Cursor, and Grok. Uses the same catalog tools as in-app agents (terminal, SFTP, Vault, port forwarding). Keep MagiesTerminal running while clients are connected.',
  ...
  'ai.externalMcp.mode.description': 'Temporary mode auto-disables after idle timeout. Always-on restores External MCP when MagiesTerminal starts.',
  ...
  'ai.externalMcp.usage.keepRunning': '1. Turn on External MCP and keep MagiesTerminal running.',
  ...
  'ai.externalMcp.permissionMode.hint': 'Same setting as Settings → AI → Safety. Auto runs MagiesTerminal write tools without MagiesTerminal approval prompts; Confirm asks each time. External clients (Codex / Claude / Grok) may still show their own tool approval UI.',
  ...
  'ai.externalMcp.conflict.description': 'A magiesTerminal-external entry already exists and points elsewhere. Remove or edit it manually.',
  ...
  'ai.userSkills.description': 'Open the MagiesTerminal skills folder to add your own skill directories. MagiesTerminal scans these skills automatically and injects only lightweight indexes unless a skill clearly matches the current request.',
  ...
  // AI Error
  'ai.codex.bridgeError': 'Codex main-process handlers are not loaded yet. Fully restart MagiesTerminal, or restart the Electron dev process, then try again.',
  ...
  'ai.safety.permissionMode.description': 'Controls how the AI interacts with your MagiesTerminal terminal sessions. Observer mode blocks write operations that go through MagiesTerminal. External agent CLIs may still have their own local tools and approval flow.',
  ...
  'ai.safety.commandTimeout.description': 'Maximum seconds a command can run before being terminated through MagiesTerminal execution.',
  ...
  'ai.safety.blocklist.description': 'Regex patterns to block dangerous commands executed through MagiesTerminal.',
  ...
  'ai.safety.note': 'These safety settings are enforced for actions that go through MagiesTerminal. External agent CLIs may also expose local tools that are governed by the agent itself.',
