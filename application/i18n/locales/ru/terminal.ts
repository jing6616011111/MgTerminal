...
export const ruTerminalMessages: Messages = {
  ...
  'terminal.osc7Setup.desc': 'MagiesTerminal добавит хуки OSC 7 для текущего удалённого пользователя. Это помогает SFTP следовать за каталогом терминала после sudo или su.',
  ...
  'cloudSync.strategy.smartMergeDesc': 'По возможности объединять изменения с обеих сторон; если MagiesTerminal не сможет безопасно выбрать, он попросит вас решить вручную.',
  ...
  'cloudSync.localBackups.desc': 'MagiesTerminal сохраняет локальные точки восстановления перед сменой версии приложения и перед восстановлением хранилища.',
  'cloudSync.localBackups.retentionTitle': 'Хранение резервных копий',
  'cloudSync.localBackups.retentionDesc': 'Выберите, сколько локальных резервных копий должен хранить MagiesTerminal.',
  ...
  'cloudSync.localBackups.unavailableDesc': 'Эта платформа не предоставляет MagiesTerminal безопасное хранилище ключей, поэтому локальные резервные копии нельзя записывать безопасно. Установите MagiesTerminal в систему с поддерживаемым keychain, чтобы включить историю локальных резервных копий.',
  ...
  'cloudSync.githubFlow.desc': 'Скопируйте код ниже и введите его на GitHub, чтобы авторизовать MagiesTerminal.',
  ...
  'snippets.import.toast.invalidDesc': 'Это невалидный файл сниппетов MagiesTerminal.',
  ...
  'snippets.import.modal.desc': 'Выберите один или несколько JSON-файлов сниппетов MagiesTerminal. MagiesTerminal сначала покажет результат разбора и ничего не импортирует без подтверждения.',
