---
name: magies-terminal-tool-cli
description: Use this skill when an external agent needs to operate on MagiesTerminal sessions through Skills + CLI instead of the magiesTerminal-remote-hosts MCP server.
---

# MagiesTerminal Tool CLI

Use this skill for external ACP agents when MagiesTerminal is configured for `Skills + CLI` mode.

For routine tasks, the host prompt is usually enough. Read only the reference that matches the task type.

## Router

1. Use the exact MagiesTerminal CLI prefix provided by the host prompt.
2. Keep `--chat-session <chat-session-id>` on every MagiesTerminal CLI call. Do not omit it.
...
- Run MagiesTerminal CLI commands strictly serially.
- Treat MagiesTerminal CLI errors as authoritative.
- Never ask the user for SSH credentials, key paths, proxy settings, or jump-host details when MagiesTerminal session access already exists.
...
- When the execution surface accepts argv-style calls, use the MagiesTerminal launcher path as the executable and pass subcommands and flags as separate arguments instead of wrapping it in another shell.
