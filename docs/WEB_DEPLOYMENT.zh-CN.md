...
脚本会自动判断权限：当前是 `root` 时直接执行；普通用户运行时，脚本会在需要安装软件或操作 Docker 时调用 `sudo`。因此命令中不需要手动添加 `sudo bash`。
...
```bash
curl -fsSL https://raw.githubusercontent.com/jing6616011111/MgTerminal/main/scripts/install-web.sh \
  | MGTERMINAL_ADMIN_PASSWORD='至少12位的强密码' bash
```
...
普通用户如果没有 Docker 操作权限，可在 `docker` 命令前添加 `sudo`；当前已经是 `root` 时不要添加。
...
上述命令以 `root` 为例；普通用户没有 Docker 权限时在命令前添加 `sudo`。
