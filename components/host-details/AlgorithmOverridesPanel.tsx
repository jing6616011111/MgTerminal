...
interface Props {
   ...
   * here means the host can freely use MagiesTerminal defaults by resetting
   ...
   * jump them to MagiesTerminal's defaults in that case.
   */
 ...
 *
 * When a category's array is `undefined`, that category uses MagiesTerminal's
...
export const AlgorithmOverridesPanel: React.FC<Props> = ({
  ...
  const toggleAlgorithm = useCallback(
    (category: SSHAlgorithmCategory, algo: string) => {
      ...
      if (!current) {
        ...
        // group's narrowing). Otherwise seed from MagiesTerminal's effective
  ...
  const isChecked = useCallback(
    (category: SSHAlgorithmCategory, algo: string) => {
      ...
      // them, and the runtime falls back to MagiesTerminal defaults.
