 ...
 *     MagiesTerminal already holds (stored password, parseable private key,
 ...
 *     "trusted" in MagiesTerminal's known-hosts store. A host verifier classifies
...
function createMoshStatsConnectionApi(ctx) {
  ...
  with (ctx) {
    ...
    // key is already trusted — by MagiesTerminal's in-app known-hosts store OR by the
    ...
    // host key in `~/.ssh/known_hosts`. MagiesTerminal's vault snapshot is NOT updated
    ...
    function createTrustEnforcingHostVerifier({ hostname, port, knownHosts, verifyHostKeys = true, trust, label }) {
      return (rawKey, callback) => {
        ...
        try {
          ...
          // source) only when MagiesTerminal's snapshot does not already vouch for
    ...
    async function buildStatsConnectOpts(auth, label = "Mosh") {
      ...
      // authenticate to or run commands against a host MagiesTerminal has not vetted
