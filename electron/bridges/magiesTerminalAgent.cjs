/**
 * MagiesTerminal in-process SSH agent
 *
...
const DEBUG_SSH = process.env.MAGIES_TERMINAL_SSH_DEBUG === "1";

// Debug logger (disabled by default)
const logFile = DEBUG_SSH
  ? path.join(require("os").tmpdir(), "magiesTerminal-agent.log")
  : null;
const log = (msg, data) => {
  if (!DEBUG_SSH) return;
...
class MagiesTerminalAgent extends BaseAgent {
  constructor(opts) {
    super();
