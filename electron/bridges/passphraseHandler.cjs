...
function cancelPassphraseRequest(requestId, reason = "cancelled") {
  const cancelled = settleRequest(
    ...
    { cancelled: true },
    {
      channel: "magiesTerminal:passphrase-cancelled",
      payload: { reason },
    }
  );
  if (cancelled) {
...
function requestPassphrase(sender, keyPath, keyName, hostname, passphraseInvalid, options = {}) {
  return new Promise((resolve) => {
    ...
    const timeoutId = setTimeout(() => {
      if (passphraseRequests.has(requestId)) {
        ...
        settleRequest(
          requestId,
          null,
          { channel: "magiesTerminal:passphrase-timeout" }
        );
      }
    }, REQUEST_TTL_MS);
    ...
    try {
      sender.send('magiesTerminal:passphrase-request', {
        requestId,
        keyPath,
        keyName,
        hostname,
...
function registerHandler(ipcMain) {
  ipcMain.handle('magiesTerminal:passphrase:respond', handleResponse);
}
