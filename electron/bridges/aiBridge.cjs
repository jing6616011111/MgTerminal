...
const MAGIES_TERMINAL_TOOL_SKILL_PATH = toUnpackedAsarPath(
  path.resolve(__dirname, "../../skills/magies-terminal-tool-cli/SKILL.md"),
);
const MAGIES_TERMINAL_TOOL_LAUNCHER_PATH = getCliLauncherPath();
const MAGIES_TERMINAL_TOOL_CLI_PATH = toUnpackedAsarPath(
  path.resolve(__dirname, "../cli/magies-terminal-tool-cli.cjs"),
);
...
function getSkillsCliInvocation() {
  if (existsSync(MAGIES_TERMINAL_TOOL_LAUNCHER_PATH)) {
    return {
      commandPrefix: `"${MAGIES_TERMINAL_TOOL_LAUNCHER_PATH}"`,
      launcherPath: MAGIES_TERMINAL_TOOL_LAUNCHER_PATH,
      usesLauncher: true,
    };
  }
  if (existsSync(MAGIES_TERMINAL_TOOL_CLI_PATH)) {
    return {
      commandPrefix: `node "${MAGIES_TERMINAL_TOOL_CLI_PATH}"`,
      launcherPath: null,
      usesLauncher: false,
    };
  }
  return {
    commandPrefix: "magies-terminal-tool-cli",
    launcherPath: null,
    usesLauncher: false,
  };
}
...
function buildExternalAgentSystemContext({ mode, chatSessionId, defaultTargetSession, userSkillsContext }) {
  ...
  if (mode === "skills") {
    ...
    const skillHint = existsSync(MAGIES_TERMINAL_TOOL_SKILL_PATH)
      ? `The local MagiesTerminal skill file is "${MAGIES_TERMINAL_TOOL_SKILL_PATH}". You do not need to read it for routine read-only requests if the host instructions here are sufficient. Only open it when the task is unusual, multi-step, or you are unsure about the workflow. `
      : "";
    const cliHint = usesLauncher
      ? (
        `For this chat session, the MagiesTerminal CLI launcher is at \`${launcherPath}\`. ` +
        `Invoke that launcher directly for every MagiesTerminal CLI call, and do not prepend \`node\`. ` +
      ...
      : existsSync(MAGIES_TERMINAL_TOOL_CLI_PATH)
        ? `For this chat session, the exact MagiesTerminal CLI command prefix is \`${cliCommandPrefix}\`.`
        : "Use the exact MagiesTerminal CLI command prefix provided by the host application for this chat session. ";
    const scopeHint = chatSessionId
      ? `Always include \`--chat-session ${chatSessionId}\` on every MagiesTerminal CLI call so you stay inside the current scoped session set. `
      : "";
    ...
    return (
      `${userSkillsPreamble}` +
      `[Context: You are inside MagiesTerminal, a multi-session terminal manager. ` +
      `${skillHint}` +
      `${cliHint}` +
      `${scopeHint}` +
      `${defaultTargetHint}` +
      `Use Skills + CLI instead of the "magiesTerminal-remote-hosts" MCP server for MagiesTerminal session access. ` +
      `Use the local shell only to invoke MagiesTerminal CLI commands or inspect local attachments explicitly supplied by the user. Do not use local shell or filesystem tools for unrelated local-machine work. ` +
      ...
      `Run MagiesTerminal CLI calls strictly one at a time. Do not issue concurrent or background MagiesTerminal CLI commands for the same chat session, and always wait for each call to finish before starting the next one. ` +
      ...
      `Avoid shell command substitution such as \`$()\` and backticks, because MagiesTerminal safety policy may block them. Prefer straightforward command chains such as \`hostname && hostname -I && lscpu\`. ` +
      ...
      `If the user stops the run or asks to abort outstanding MagiesTerminal work, use \`${cliCommandPrefix} cancel --chat-session ${chatSessionId || "<chat-session-id>"} --json\`, and use \`resume\` to re-enable execs for that scope if needed. ` +
  ...
  return appendVaultAgentGuidance(
    `${userSkillsPreamble}` +
    `[Context: You are inside MagiesTerminal, a multi-session terminal manager. ` +
    `Use the "magiesTerminal-remote-hosts" MCP tools to operate only on the terminal sessions exposed by MagiesTerminal. ` +
      ...
      (res) => {
        ...
        if (statusCode < 200 || statusCode >= 300) {
          ...
          res.on("end", () => {
            ...
            safeSend(event.sender, "magiesTerminal:ai:stream:error", {
              requestId,
        ...
        res.on("data", (chunk) => {
          ...
          for (const line of lines) {
            ...
            if (trimmed.startsWith("data: ")) {
              safeSend(event.sender, "magiesTerminal:ai:stream:data", {
                requestId,
        ...
        res.on("end", () => {
          ...
          safeSend(event.sender, "magiesTerminal:ai:stream:end", { requestId });
          activeStreams.delete(requestId);
        });
...
function createHandlerContext(ipcMain) {
  return {
    ...
    setCodexValidationCache,
    DEBUG_MCP,
    MAGIES_TERMINAL_TOOL_SKILL_PATH,
    MAGIES_TERMINAL_TOOL_LAUNCHER_PATH,
    MAGIES_TERMINAL_TOOL_CLI_PATH,
    debugMcpLog,
