...
/**
 * Build the magies-terminal-mcp-server config to inject into an SDK agent as an
 ...
 *
 * Returns an array of magiesTerminal MCP server configs (0 or 1 entry):
...
async function buildInjectedMcpServers({
  ...
  try {
    // Start the magiesTerminal control host for BOTH modes. getOrCreateHost binds the
    // TCP server and writes the magies-terminal-tool-cli discovery file on bind:
    ...
    //     the magiesTerminal CLI. Skipping this in skills mode left no host for the CLI
    //     to find, so every `magies-terminal-tool-cli` call failed with APP_NOT_RUNNING.
    const mcpPort = await mcpServerBridge.getOrCreateHost();
    // Skills mode drives the magiesTerminal CLI, not an injected MCP server.
    if (toolIntegrationMode !== "mcp") return [];
    const scopedIds = mcpServerBridge.getScopedSessionIds(chatSessionId);
    const magiesTerminalMcpConfig = mcpServerBridge.buildMcpServerConfig(
      mcpPort,
      scopedIds,
      chatSessionId,
    );
    return [magiesTerminalMcpConfig];
  } catch (err) {
    console.error("[sdk] Failed to ensure magiesTerminal host / inject MCP server:", err?.message || err);
    return [];
  }
}
