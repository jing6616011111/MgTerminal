/**
 * MagiesTerminal MCP Server (stdio transport)
 *
 * Spawned by managed SDK agents as a child process.
 * Communicates with the MagiesTerminal main process via TCP (JSON-RPC over newline-delimited JSON).
 * Exposes MagiesTerminal terminal tools so external agents can operate on scoped sessions.
 */
"use strict";
...
function debugLog(...args) {
  if (!DEBUG_MCP) return;
  process.stderr.write(`[magiesTerminal-mcp:debug] ${args.map(arg => {
...
// ── TCP Bridge to MagiesTerminal main process ──

const MAGIES_TERMINAL_MCP_PORT = parseInt(process.env.MAGIES_TERMINAL_MCP_PORT, 10);
if (!MAGIES_TERMINAL_MCP_PORT) {
  process.stderr.write("[magiesTerminal-mcp] MAGIES_TERMINAL_MCP_PORT not set\n");
  process.exit(1);
}

// Auth token for TCP bridge authentication
const MAGIES_TERMINAL_MCP_TOKEN = process.env.MAGIES_TERMINAL_MCP_TOKEN || "";
if (!MAGIES_TERMINAL_MCP_TOKEN) {
  process.stderr.write("[magiesTerminal-mcp] MAGIES_TERMINAL_MCP_TOKEN not set\n");
  process.exit(1);
}
...
const SCOPED_SESSION_IDS = process.env.MAGIES_TERMINAL_MCP_SESSION_IDS != null
  ? process.env.MAGIES_TERMINAL_MCP_SESSION_IDS.split(",").map(s => s.trim()).filter(Boolean)
  : null;

// Chat session ID for per-scope metadata isolation
const CHAT_SESSION_ID = process.env.MAGIES_TERMINAL_MCP_CHAT_SESSION_ID || null;
...
function readPermissionMode() {
  const discoveryPath = process.env.MAGIES_TERMINAL_EXTERNAL_MCP_DISCOVERY_FILE;
  if (discoveryPath) {
    try {
  ...
  return process.env.MAGIES_TERMINAL_MCP_PERMISSION_MODE || "confirm";
}
...
function connectTcp() {
  return new Promise((resolve, reject) => {
    const sock = net.createConnection({ host: "127.0.0.1", port: MAGIES_TERMINAL_MCP_PORT }, () => {
      tcpSocket = sock;
      debugLog("Connected to TCP bridge", { port: MAGIES_TERMINAL_MCP_PORT });
      resolve();
    });
    ...
    sock.on("data", (chunk) => {
      ...
      if (tcpBuffer.length > MAX_BUFFER_SIZE) {
        process.stderr.write(`[magiesTerminal-mcp] TCP buffer exceeded ${MAX_BUFFER_SIZE} bytes, clearing buffer\n`);
        tcpBuffer = "";
        return;
      }
      let newlineIdx;
...
function rpcCall(method, params) {
  return new Promise((resolve, reject) => {
    if (!tcpSocket || tcpSocket.destroyed) {
      return reject(new Error("Not connected to MagiesTerminal"));
    }
    const id = nextRpcId++;
...
server.resource(
  "environment",
  "magies-terminal://context",
  { description: "Current MagiesTerminal workspace context: connected hosts, session IDs, and environment description." },
  async () => {
    const ctx = await rpcCall("magiesTerminal/getContext", scopeParams);
    return {
      contents: [{
        uri: "magies-terminal://context",
...
async function main() {
  debugLog("Starting MCP server", {
    port: MAGIES_TERMINAL_MCP_PORT,
    hasToken: Boolean(MAGIES_TERMINAL_MCP_TOKEN),
  ...
  process.stderr.write("[magiesTerminal-mcp] Authenticated with TCP bridge\n");
...
main().catch((err) => {
  process.stderr.write(`[magiesTerminal-mcp] Fatal: ${err.message}\n`);
  process.exit(1);
});
