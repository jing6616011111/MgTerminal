...
const { connectClient, createError } = require("./magiesTerminalRpcClient.cjs");
const {
...
function printHelp() {
  ...
  process.stdout.write(
    "MagiesTerminal Tool CLI\n\n" +
    "Usage:\n" +
    catalogLines + "\n\n" +
    "Examples:\n" +
    "  magies-terminal-tool-cli status --json\n" +
    "  magies-terminal-tool-cli env --chat-session ai_123 --json\n" +
    "  magies-terminal-tool-cli session --session sess_123 --json --chat-session ai_123\n" +
    "  magies-terminal-tool-cli exec --session sess_123 --chat-session ai_123 --json -- \"pwd\"\n" +
...
function formatEnvText(ctx) {
  const header = [
    `Environment: ${ctx.environment || "magiesTerminal-terminal"}`,
...
async function resolveTargetHost(client, opts) {
  const ctx = await client.call("magiesTerminal/getContext", buildScopeParams(opts));
  if (opts.sessionId) {
...
async function run() {
  ...
  try {
    ...
    if (command === "env") {
      ...
      const result = await client.call("magiesTerminal/getContext", params);
    ...
    if (command === "exec") {
      ...
      const result = await client.call("magiesTerminal/exec", rpcParams);
      if (result.ok === false) {
    ...
    if (command === "job-start") {
      ...
      const host = await resolveTargetHost(client, opts);
      const result = await client.call("magiesTerminal/jobStart", {
        sessionId: host.sessionId,
        command: shellCommand,
        chatSessionId: opts.chatSessionId,
      });
    ...
    if (command === "job-poll") {
      ...
      const result = await client.call("magiesTerminal/jobPoll", {
        jobId: opts.jobId,
        offset,
        chatSessionId: opts.chatSessionId,
    ...
    if (command === "job-stop") {
      ...
      const result = await client.call("magiesTerminal/jobStop", {
        jobId: opts.jobId,
        chatSessionId: opts.chatSessionId,
        ...buildScopeParams(opts),
      });
    ...
    if (command === "sftp") {
      ...
      if (subcommand === "list") {
        ...
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/list", buildSftpParams()),
          "SFTP_LIST_FAILED",
      ...
      if (subcommand === "read") {
        ...
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/read", buildSftpParams()),
          "SFTP_READ_FAILED",
      ...
      if (subcommand === "write") {
        ...
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/write", buildSftpParams()),
          "SFTP_WRITE_FAILED",
      ...
      if (subcommand === "download") {
        ...
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/download", buildSftpParams()),
      ...
      if (subcommand === "upload") {
        ...
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/upload", buildSftpParams()),
          "SFTP_UPLOAD_FAILED",
      ...
      if (subcommand === "mkdir") {
        ...
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/mkdir", buildSftpParams()),
          "SFTP_MKDIR_FAILED",
      ...
      if (subcommand === "delete") {
        ...
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/delete", buildSftpParams()),
          "SFTP_DELETE_FAILED",
      ...
      if (subcommand === "rename") {
        ...
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/rename", buildSftpParams()),
          "SFTP_RENAME_FAILED",
      ...
      if (subcommand === "stat") {
        ...
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/stat", buildSftpParams()),
          "SFTP_STAT_FAILED",
      ...
      if (subcommand === "chmod") {
        ...
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/chmod", buildSftpParams()),
          "SFTP_CHMOD_FAILED",
      ...
      if (subcommand === "home") {
        const result = ensureBridgeCallOk(
          await client.call("magiesTerminal/sftp/home", buildSftpParams()),
          "SFTP_HOME_FAILED",
    ...
    if (command === "cancel" || command === "resume") {
      ...
      const result = await client.call("magiesTerminal/setCancelled", {
        chatSessionId: opts.chatSessionId,
        cancelled,
      });
