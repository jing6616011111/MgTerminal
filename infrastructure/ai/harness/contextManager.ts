...
function buildReinjectionMessages(reinjection?: PostCompactReinjection): ModelMessage[] {
  ...
  const lines: string[] = ['[MagiesTerminal session context — preserved after compaction]'];
...
export function extractLatestUserGoal(messages: ModelMessage[] | ChatMessage[]): string | undefined {
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    ...
    const content = typeof message.content === 'string'
      ...
      : '';
    if (content && !content.startsWith('[MagiesTerminal session context')) return content.slice(0, 500);
  }
  return undefined;
}
