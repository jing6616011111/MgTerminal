...
function setBridge(bridge: Record<string, unknown>): () => void {
  ...
  // Loosely typed: the real window.magiesTerminal is a large MagiesTerminalBridge; tests
  // only stub the handful of OneDrive methods the adapter actually calls.
  g.window = { magiesTerminal: bridge } as unknown as Window & typeof globalThis;
  return () => {
    g.window = original;
  };
}
